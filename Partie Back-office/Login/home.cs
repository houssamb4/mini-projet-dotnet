...
namespace Login
{
    public partial class home : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (IsFormValid())
            {
                if (client.AjouterClient(txtNom.Text, txtEmail.Text, txtPhone.Text, txtAdresse.Text))
                {
                    ...
                    SelectClient(); // Refresh the client list
                }
                else
                {
        ...
        {
            SelectClient(); // Load clients on form load
        }
        ...
        private void SelectClient()
        {
            try
            {
                ...
                while (result.Read())
                {
                    tableauClient.Rows.Add(result.GetValue(0).ToString(), result.GetValue(1).ToString(), result.GetValue(2).ToString(), result.GetValue(3).ToString(), result.GetValue(4).ToString());
                }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (tableauClient.SelectedRows.Count > 0)
            {
                ...
                {
                    string id = tableauClient.CurrentRow.Cells[0].Value.ToString(); // Accessing the 'id' column by index
                    string nom = tableauClient.CurrentRow.Cells[1].Value.ToString(); // Accessing the 'nom_complet' column by index
        ...
        private void btnUpdateClt_Click(object sender, EventArgs e)
        {
            try
            {
                if (IsFormValid())
                {
                    string id = tableauClient.CurrentRow.Cells[0].Value.ToString(); // Ensure 'id' is correctly accessed
        ...
        {
            if (e.RowIndex >= 0)  // Ensure it's not the header
            {
